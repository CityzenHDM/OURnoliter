...
# [추가] 어드민 뷰 상태 관리
if "admin_view" not in st.session_state: st.session_state.admin_view = "main"

# --- 3. 검증 함수들 ---
def verify_password(email, password):
    if not FIREBASE_WEB_API_KEY: return False
...
def is_valid_image_url(url):
    ...
    if not url.lower().endswith((".png", ".jpg", ".jpeg", ".gif", ".webp")): return False
    try: return requests.head(url, timeout=3).status_code == 200
    except: return False

# --- 4. CSS 및 상단바 (어드민 전용 버튼 추가) ---
# --- 4. CSS 및 상단바 (디자인 수정) ---
...
st.markdown("""
    ...
    .not-found-box { background-color: #fee2e2; border: 1px solid #ef4444; color: #b91c1c; padding: 15px; border-radius: 10px; text-align: center; margin-bottom: 10px; }
    ...
    .profile-img { width: 35px; height: 35px; border-radius: 50%; object-fit: cover; margin-right: 8px; border: 1px solid #ddd; vertical-align: middle; }
    ...
    .rank-item { padding: 8px; border-bottom: 1px solid #eee; display: flex; align-items: center; justify-content: space-between; }
    .post-meta { display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px; padding-bottom: 5px; border-bottom: 1px dashed #eee; }
    </style>
    """, unsafe_allow_html=True)

# 상단바 렌더링 (HTML 태그 노출 문제 해결)
st.markdown('<div class="top-header">OurNoliter.com</div>', unsafe_allow_html=True)

# 어드민 전용 버튼 배치 (상단바 바로 아래)
is_admin = st.session_state.user_info and st.session_state.user_info['email'] == ADMIN_EMAIL
if is_admin:
    col_left, col_mid, col_right = st.columns([0.3, 0.4, 0.3])
    with col_mid:
        if st.button("👥 유저 리스트 확인", use_container_width=True):
            st.session_state.admin_view = "user_list"
            st.rerun()
...
# --- 5. 화면 분기 (유저 리스트 vs 메인) ---
if st.session_state.admin_view == "user_list" and is_admin:
    user_management.show_user_list()
else:
    # --- 사이드바 ---
    with st.sidebar:
        ...
        if st.session_state.user_info is None:
            ...
            with menu[0]:
                ...
                if st.button("로그인", use_container_width=True, type="primary"):
                    if verify_password(l_email, l_pw):
                        ...
                        st.session_state.user_info = {"name": user.display_name, "email": user.email, "photo": user.photo_url or DEFAULT_AVATAR}
            ...
            if st.button("로그아웃", use_container_width=True):
                st.session_state.user_info = None; st.session_state.admin_view = "main"; st.rerun()

        st.markdown("---")
        st.subheader("📁 놀이터 목록")
        if st.button("🏠 전체 메인 피드", use_container_width=True):
            st.session_state.current_playground = "전체"; st.rerun()
        ...
        for pg in pg_list_side:
            pg_name = pg.id; existing_pg_names.append(pg_name)
            c1, c2 = st.columns([0.8, 0.2]) if is_admin else (st.container(), None)
            with (c1 if is_admin else st.container()):
    ...
    # --- 데이터 집계 및 메인 화면 (게시글/랭킹 로직 등... 기존과 동일) ---
    ...
    with col_s: s_in = st.text_input("놀이터 검색", placeholder="가고 싶은 놀이터...", key="search_bar", label_visibility="collapsed")
    with col_b:
    ...
    if st.session_state.search_target:
        ...
        if st.session_state.user_info and st.button(f"🏗️ '{st.session_state.search_target}' 만들기", use_container_width=True):
    ...
    if curr == "전체":
        ...
        with c1:
            ...
            for i, (name, count) in enumerate(sorted_pg_ranks[:10]):
                st.markdown(f'<div class="rank-item"><span><span class="rank-badge">{i+1}위</span> 🎡 {name}</span><span style="color:gray; font-size:12px;">{count} posts</span></div>', unsafe_allow_html=True)
        with c2:
            st.subheader("🎡 바로가기")
        ...
        for post in posts:
            ...
            score = len(up_l) - len(down_l)
            with st.expander(f"📄 {p.get('title')}  (⭐ {score})"):
                st.markdown(f"""<div class="post-meta"><div><img src="{p.get('author_photo', DEFAULT_AVATAR)}" class="profile-img"><b>{p.get('author')}</b><small style="color:gray; margin-left:10px;">{p['created_at'].strftime('%Y-%m-%d %H:%M')}</small></div></div>""", unsafe_allow_html=True)
                if p.get('image'): st.image(p['image'], use_container_width=True)
                st.write(p.get('content'))
                b1, b2, b3, _ = st.columns([0.2, 0.2, 0.2, 0.4])
                with b1:
                ...
                for idx, com in enumerate(comments):
                    ...
                    with c_col1: st.markdown(f'**{com["author"]}**: {com["text"]}')
                    with c_col2:
                        if st.button(f"❤️ {len(c_hearts)}", key=f"h_com_{pid}_{idx}"):
                            if st.session_state.user_info:
                                u_email = st.session_state.user_info['email']
                                if u_email in c_hearts: c_hearts.remove(u_email)
                                else: c_hearts.append(u_email)
                                comments[idx]['hearts'] = c_hearts
                                db.collection("posts").document(pid).update({"comments": comments}); st.rerun()
                if st.session_state.user_info:
                    with st.form(key=f"f_{pid}", clear_on_submit=True):
                        ...
                        if st.form_submit_button("댓글 등록") and c_txt:
                            new_c = {"author": st.session_state.user_info['name'], "text": c_txt, "hearts": []}
